defmodule Caudata.UI.Components.AddServerModal do
  ...
  def render(state) do
    case state.modal_type do
      :select_ssh ->
        options = [
          {"+ Manual Connection", :manual} | Enum.map(state.ssh_config_profiles, &{&1.id, &1})
        ]
  ...
  defp handle_select_ssh_key(key, key_data, model) do
    ...
    total_options = 1 + length(model.ssh_config_profiles)
    ...
    case norm_key do
      ...
      :enter ->
        if model.modal_selected_index == 0 do
          ...
          profile = Enum.at(model.ssh_config_profiles, model.modal_selected_index - 1)
