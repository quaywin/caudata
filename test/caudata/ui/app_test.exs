defmodule Caudata.UI.AppTest do
  ...
  test "handle_event/2 handles keyboard events for logs scrolling" do
    ...
    state = %{state | logs: logs, height: 24}
  ...
  test "handle_event/2 handles loading older logs when scrolling up with k" do
    ...
    Caudata.LogStore.append_logs(profile_id, logs_history)
    ...
    # Assert that fetch limit increased
    assert updated_state.logs_fetch_limit == 1020
    # Assert logs was populated with all 35 logs from the store
    assert length(updated_state.logs) == 35

    # Since 15 new historical lines were prepended, new_scroll should be min(35-20, max(0, 15-3)) = min(15, 12) = 12
    assert updated_state.logs_scroll_y == 12
  end
end
