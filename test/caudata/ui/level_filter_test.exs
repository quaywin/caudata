defmodule Caudata.UI.LevelFilterTest do
  ...
  test "LevelFilterModal renders all 4 severity options" do
    ...
    assert Enum.any?(text_contents, &String.contains?(&1, "[1] Info only"))
    assert Enum.any?(text_contents, &String.contains?(&1, "[2] Warnings only"))
    assert Enum.any?(text_contents, &String.contains?(&1, "[3] Errors & Fatal"))
  ...
  test "ViewHelper filters logs by exact severity level" do
    logs = [
      ...
      %{timestamp: "2026-08-21T06:00:05Z", stream: :stdout, message: "Plain text log without level"}
    ]
    ...
    # All logs (6)
    assert length(ViewHelper.get_displayed_logs(base_model)) == 6

    # INFO only (1 log: info)
    assert length(ViewHelper.get_displayed_logs(%{base_model | log_level_filter: :info})) == 1

    # WARN only (1 log: warn)
    assert length(ViewHelper.get_displayed_logs(%{base_model | log_level_filter: :warn})) == 1
